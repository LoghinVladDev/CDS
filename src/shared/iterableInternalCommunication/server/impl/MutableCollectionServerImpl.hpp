// NOLINT(llvm-header-guard)
// Created by loghin on 7/3/22.
//

#ifndef __CDS_SHARED_MUTABLE_COLLECTION_SERVER_IMPL_HPP__   // NOLINT(llvm-header-guard)
#define __CDS_SHARED_MUTABLE_COLLECTION_SERVER_IMPL_HPP__   // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
#pragma once

#ifdef _MSC_VER
#include "../../../impl/forceFunctionPointerCast.hpp"
#endif

namespace cds {       // NOLINT(modernize-concat-nested-namespaces) 
namespace __hidden {  // NOLINT(modernize-concat-nested-namespaces, bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp) 
namespace __impl {    // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
template <typename __Receiver, typename __Element>  // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
typename __MutableCollectionServer <__Receiver, __Element>::__GenericHandler const __MutableCollectionServer <  // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp, *-c-arrays)
    __Receiver, __Element
>::__routingTable [static_cast <uint32> (__IterableInternalRequest::__iir_requestCountDoNotUse)] = {  // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp, *-c-arrays)
    bitCast <__GenericHandler> (&__Receiver::__begin),              // __cirt_begin                         - expected part of MutableCollection          // NOLINT(*-reinterpret-cast) 
    bitCast <__GenericHandler> (&__Receiver::__end),                // __cirt_end                           - expected part of MutableCollection          // NOLINT(*-reinterpret-cast)
    nullptr,                                                                            // __cirt_rbegin                        - not expected part of MutableCollection    
    nullptr,                                                                            // __cirt_rend                          - not expected part of MutableCollection
    bitCast <__GenericHandler> (&__Receiver::__remove),             // __cirt_remove                        - expected part of MutableCollection          // NOLINT(*-reinterpret-cast) 
    bitCast <__GenericHandler> (&__Receiver::__removeConst),        // __cirt_removeConst                   - expected part of MutableCollection          // NOLINT(*-reinterpret-cast) 
    bitCast <__GenericHandler> (&__Receiver::__removeArray),        // __cirt_removeArray                   - expected part of MutableCollection          // NOLINT(*-reinterpret-cast) 
    bitCast <__GenericHandler> (&__Receiver::__removeConstArray),   // __cirt_removeConstArray              - expected part of MutableCollection          // NOLINT(*-reinterpret-cast) 
    bitCast <__GenericHandler> (&__Receiver::__newAddress),         // __cirt_newAddress                    - expected part of MutableCollection          // NOLINT(*-reinterpret-cast)
    nullptr,                                                                            // __cirt_newFrontAddress               - not expected part of MutableCollection    
    nullptr,                                                                            // __cirt_newBackAddress                - not expected part of MutableCollection    
    nullptr,                                                                            // __cirt_newFrontAddressArray          - not expected part of MutableCollection    
    nullptr,                                                                            // __cirt_newBackAddressArray           - not expected part of MutableCollection    
    nullptr,                                                                            // __cirt_newBeforeAddress              - not expected part of MutableCollection    
    nullptr,                                                                            // __cirt_newBeforeAddressConst         - not expected part of MutableCollection    
    nullptr,                                                                            // __cirt_newAfterAddress               - not expected part of MutableCollection    
    nullptr,                                                                            // __cirt_newAfterAddressConst          - not expected part of MutableCollection    
    nullptr,                                                                            // __cirt_newBeforeAddressArray         - not expected part of MutableCollection    
    nullptr,                                                                            // __cirt_newBeforeAddressArrayConst    - not expected part of MutableCollection    
    nullptr,                                                                            // __cirt_newAfterAddressArray          - not expected part of MutableCollection    
    nullptr,                                                                            // __cirt_newAfterAddressArrayConst     - not expected part of MutableCollection    
    nullptr,                                                                            // __cirt_find                          - not expected part of MutableCollection    
    nullptr,                                                                            // __cirt_findConst                     - not expected part of MutableCollection    
};


template <typename __Receiver, typename __Element>  // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
typename __MutableCollectionServer <__Receiver, __Element>::__GenericConstHandler const __MutableCollectionServer < // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp, *-c-arrays)
    __Receiver, __Element
>::__constRoutingTable [static_cast <uint32> (__IterableInternalRequest::__iir_requestCountDoNotUse)] = {  // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp, *-c-arrays)
    bitCast <__GenericConstHandler> (&__Receiver::__cbegin),    // __cirt_begin                         - expected part of MutableCollection          // NOLINT(*-reinterpret-cast)
    bitCast <__GenericConstHandler> (&__Receiver::__cend),      // __cirt_end                           - expected part of MutableCollection          // NOLINT(*-reinterpret-cast)
    nullptr,                                                                        // __cirt_rbegin                        - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_rend                          - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_remove                        - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_removeConst                   - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_removeArray                   - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_removeConstArray              - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_newAddress                    - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_newFrontAddress               - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_newBackAddress                - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_newFrontAddressArray          - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_newBackAddressArray           - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_newBeforeAddress              - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_newBeforeAddressConst         - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_newAfterAddress               - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_newAfterAddressConst          - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_newBeforeAddressArray         - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_newBeforeAddressArrayConst    - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_newAfterAddressArray          - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_newAfterAddressArrayConst     - not expected part of MutableCollection
    nullptr,                                                                        // __cirt_find                          - not expected part of MutableCollection
    nullptr                                                                         // __cirt_findConst                     - not expected part of MutableCollection
};


template <typename __Receiver, typename __Element>  // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
constexpr auto __MutableCollectionServer <__Receiver, __Element>::__mcs_handlers () const noexcept -> __GenericHandler const* {
  // return the first address in the routing table 
  return &__MutableCollectionServer::__routingTable [0u];
}


template <typename __Receiver, typename __Element>  // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
constexpr auto __MutableCollectionServer <__Receiver, __Element>::__mcs_constHandlers () const noexcept -> __GenericConstHandler const* {
  // return the first address in the const routing table 
  return &__MutableCollectionServer::__constRoutingTable [0u];
}
} // namespace __impl 
} // namespace __hidden 
} // namespace cds 

#endif // __CDS_SHARED_MUTABLE_COLLECTION_SERVER_IMPL_HPP__ 