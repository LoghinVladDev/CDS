...
__CDS_OptimalInline auto __FindUniqueMutablePrimitiveClient <__Receiver, __Element, __Key, __bidirectional>::find (
    __Key const& key
) noexcept(false) -> Iterator {
  // explicit type of the handler received from the client
  using __ReceiverFindHandlerType = cds::functional::MapperFunction <__AbstractDelegateIterator <__Element>*, __Receiver*, __Key const&>;  // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
  ...
  return Iterator (pReceiver, bitCast <__ReceiverFindHandlerType> (pReceiver->__iicch_obtainGenericHandler (__IterableInternalRequest::__iir_find)) (pReceiver, key));
}
...
__CDS_OptimalInline auto __LocalFindUniqueMutablePrimitiveClient <__Receiver, __Key, __Iterator>::find (
    __Key const& key
) noexcept -> Iterator {
  // directly call the __findLocal function
  return __Receiver::__findLocal (static_cast <__Receiver*> (this), key);
}
} // namespace __impl
} // namespace __hidden
} // namespace cds
