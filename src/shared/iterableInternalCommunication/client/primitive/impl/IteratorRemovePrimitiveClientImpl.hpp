...
auto __AbstractIteratorRemovePrimitiveClient <__Receiver, __Element>::remove (
    AbstractIterator const& iterator
) noexcept(false) -> bool {
  using __ReceiverRemoveIteratorHandlerType   = cds::functional::PredicateFunction <__Receiver*, cds::meta::Iterator const*>; // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
  ...
  return bitCast <__ReceiverRemoveIteratorHandlerType> (pReceiver->__iicch_obtainGenericHandler (__IterableInternalRequest::__iir_remove)) (
...
template <typename __Receiver, typename __Element>  // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
auto __AbstractIteratorRemovePrimitiveClient <__Receiver, __Element>::removeAll (
  ...
  using __ReceiverRemoveIteratorArrayHandlerType  = cds::functional::MapperFunction <Size, __Receiver*, cds::meta::Iterator const* const*, Size>;  // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
  using __ReceiverRemoveIteratorHandlerType       = cds::functional::PredicateFunction <__Receiver*, cds::meta::Iterator const*>; // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
  ...
  try {
    ...
    if (genericIteratorArraySize >= 2u) {
      removedIteratorCount = bitCast <__ReceiverRemoveIteratorArrayHandlerType> (pReceiver->__iicch_obtainGenericHandler (__IterableInternalRequest::__iir_removeArray)) (
          pReceiver, &genericIteratorArray [0u], genericIteratorArraySize
      );
    } else if (genericIteratorArraySize == 1u) {
      removedIteratorCount = bitCast <__ReceiverRemoveIteratorHandlerType> (pReceiver->__iicch_obtainGenericHandler (__IterableInternalRequest::__iir_remove)) (
...
auto __LocalIteratorRemovePrimitiveClient <__Receiver, __Element, __Iterator>::remove (Iterator const& iterator) noexcept(false) -> bool {
  // local client, directly call the dispatcher-generated function for const removal
  return __Receiver::__remove (static_cast <__Receiver*> (this), &iterator);
}


template <typename __Receiver, typename __Element, typename __Iterator>                 // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
auto __LocalIteratorRemovePrimitiveClient <__Receiver, __Element, __Iterator>::removeAll (
  ...
  if (iteratorCount >= 2u) {
    ...
    try {
      ...
      Size removedCount = __Receiver::__removeArray (pReceiver, &pIteratorBuffer [0u], iteratorCount);
    ...
    return __Receiver::__remove (pReceiver, &pIterators [0u]) ? 1u : 0u;
  }
