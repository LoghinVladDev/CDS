...
namespace glob {
    ...
    public:
        ...
        public:
            ...
            constexpr explicit TerminalColor ( Flags const f ) noexcept : f(f) {}
            constexpr TerminalColor ( TerminalColor const & ) noexcept = default;
            static auto asList ( Flags const f ) noexcept -> cds :: String {
    #if defined(_WIN32)
                return "";
    #endif
                ...
                glob::foreach_flag(std::make_tuple(0, flagLimit), [& res, f](cds::uint64 const presentFlag){
    ...
    protected:
        [[nodiscard]] auto inline getDepthString() const noexcept -> cds :: String {
            return cds :: String("\t") * static_cast <int> (this->logDepth);
        }
    ...
    public:
        ...
        auto logOK ( cds :: String const & text ) const noexcept -> void;
        auto logOK ( const char * format, ... ) const noexcept -> void;

        auto logError ( cds :: String const & text) const noexcept -> void;
        auto logError ( const char * format, ... ) const noexcept -> void;

        auto logWarning ( cds :: String const & text) const noexcept -> void;
        auto logWarning ( const char * format, ... ) const noexcept -> void;

        auto logBold( cds :: String const & text) const noexcept -> void;
        auto logBold( const char * format, ... ) const noexcept -> void;
