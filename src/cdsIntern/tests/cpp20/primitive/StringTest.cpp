...
#define __FROM_RAW_CONSTRUCT(_typename, _initval) /* NOLINT(bugprone-reserved-identifier) */ { \
        ...
        String fromConstAddress = ( int const * ) 0x10;
...
#undef COMP_TEST
    ...
    this->executeSubtest("Replace Tests", [this, &allOk] {
        ...
        if ( std::strcmp ( "Test String"_s.replace(2, 3, "inserted value").cStr(), "Teinserted valueString" ) != 0 ) {
...
auto StringTest::execute() noexcept -> bool {
    ...
    this->executeSubtest("Constructor Tests", [this, & allOk] {
        ...
        experimental :: String b = "Test String to be copied";
        experimental :: String copy(b);
        this->log("String Constructed with Copy Constructor : Original : '%s', Copy : '%s'. Copy Diag : %s",
                  b.cStr(), copy.cStr(), copy.diag().cStr());
        if (b != copy) {
            this->logWarning("Copy String Unexpected Error");
        ...
        const char *literal = "Test StringLiteral to be copied";
        experimental :: String literalCopy(literal);
        this->log("String Constructed with StringLiteral : Original : '%s', String : '%s'. Diag : '%s'",
                  literal,
                  literalCopy.cStr(),
                  literalCopy.diag().cStr()
        ...
        std::string stdString = "std::string to be copied";
        experimental :: String stdCopy(stdString);
        this->log("String Constructed with std::string : Original : '%s', String : '%s'. Diag : %s",
                  stdString.c_str(),
                  stdCopy.cStr(),
                  stdCopy.diag().cStr()
        ...
        if (stdCopy != stdString) {
         ...
         experimental :: String defaultedString(10, 'a');
        this->log("String Constructed with default values : '%s'. Diag : %s", defaultedString.cStr(),
                  defaultedString.diag().cStr());
        ...
        std::string stdStringIt = "String that will be iterated. Should not appear";
        experimental :: String iteratedStd(stdStringIt.begin(), stdStringIt.end().operator-=(strlen(" Should not appear")));
        this->log("String Constructed with std::string iterators : Intended : '%s'. Result : '%s'. Diag : %s",
                  std::string(stdStringIt.begin(), stdStringIt.end().operator-=(strlen(" Should not appear"))).c_str(),
                  iteratedStd.cStr(),
                  iteratedStd.diag().cStr()
        ...
        experimental :: String stringIt = "String that will be iterated";
        experimental :: String iterated(stringIt.begin(), stringIt.end());
        this->log("String Constructed with String iterators : Intended : '%s', Result : '%s'. Diag : %s",
                  stringIt.cStr(),
                  iterated.cStr(),
                  iterated.diag().cStr()
        ...
        iterated = experimental :: String(stringIt.cbegin(), stringIt.cend());
        this->log("String Constructed with String const iterators : Intended : '%s', Result : '%s'. Diag : %s",
                  stringIt.cStr(),
                  iterated.cStr(),
                  iterated.diag().cStr()
        ...
        iterated = experimental :: String(stringIt.rbegin(), stringIt.rend());
        this->log("String Constructed with String reverse iterators : Intended : '%s', Result : '%s'. Diag : %s",
                  experimental :: String ( stringIt ).reverse().cStr(),
                  iterated.cStr(),
                  iterated.diag().cStr()
        );

        if (iterated != experimental :: String ( stringIt ).reverse()) {
            this->logWarning("String from String reverse iterators construction Error");
            allOk = false;
        }

        iterated = experimental :: String(stringIt.crbegin(), stringIt.crend());
        this->log(
                "String Constructed with String const reverse iterators : Intended : '%s', Result : '%s'. Diag : %s",
                experimental :: String ( stringIt ).reverse().cStr(),
                iterated.cStr(),
        ...
        experimental :: String initialized = {'S', 'a', 'm', 'p', 'l', 'e', ' ', 'S', 't', 'r', 'i', 'n', 'g'};
        this->log("String Constructed from std::initializer list. Intended : '%s', Result : '%s'. Diag : %s",
                  "Sample String",
                  initialized.cStr(),
                  initialized.diag().cStr()
        ...
        experimental :: String fromObject = cds :: String ( Integer::parse ("1234") ).cStr();
        this->log("String constructed from CDS/Object. Intended : '%s', Result : '%s'. Diag : %s", "1234", fromObject.cStr(), fromObject.diag().cStr());
...
#define __FROM_RAW_CONSTRUCT(_typename, _initval) /* NOLINT(bugprone-reserved-identifier) */ { \
        ...
        experimental :: String fromAddress = experimental :: String ( ( int * ) 0x10 );
        ...
        experimental :: String fromConstAddress = experimental :: String ( ( int const * const ) 0x10 );
    ...
    this->executeSubtest("Iterator Tests", [this, & allOk] {
        ...
        experimental :: String toIterate = "2345678";
    ...
    this->executeSubtest("Property Tests", [this, &allOk] {
        ...
        Array<experimental :: String> strings = {
                "Some", "strings", "to test the", "properties", "on"
        };
    ...
    this->executeSubtest("Memory Tests", [this, &allOk] {
        ...
        experimental :: String memoryOperated = "This is a sample string";
    ...
    this->executeSubtest("Content Functions Tests", [this, &allOk]{
        experimental :: String original = "The quick brown fox jumped over the lazy dog";
        experimental :: String toBeEmptied = original;
        ...
        experimental :: String indexed = "abcd";
        this->log("String to be looked up : '%s'", indexed.cStr());
        ...
        experimental :: String container = "Test String";

        this->log("String '%s' contains '%s' : %s", container.cStr(), experimental :: String("String").cStr(), container.contains(experimental :: String("String")) ? "true" : "false");
        this->log("String '%s' contains '%s' : %s", container.cStr(), experimental :: String("Integer").cStr(), container.contains(experimental :: String("Integer")) ? "true" : "false");
        this->log("String '%s' contains '%s' : %s", container.cStr(), "String", container.contains("String") ? "true" : "false");
        this->log("String '%s' contains '%s' : %s", container.cStr(), "Integer", container.contains("Integer") ? "true" : "false");
        this->log("String '%s' contains '%s' : %s", container.cStr(), std::string("String").c_str(), container.contains(std::string("String")) ? "true" : "false");
        this->log("String '%s' contains '%s' : %s", container.cStr(), std::string("Integer").c_str(), container.contains(std::string("Integer")) ? "true" : "false");
        ...
        if ( !container.contains(experimental :: String("String")) ) {
            this->logWarning("Contains Error");
            allOk = false;
        }

        if ( container.contains(experimental :: String("Integer")) ) {
            this->logWarning("Contains Error");
            allOk = false;
        }
    ...
    this->executeSubtest("Append/Prepend Tests", [this, &allOk] {
        experimental :: String original = "test string,";
        experimental :: String sa = "appended string";
        std::string stdsa = "appended std::string";
        char ca = '\\';
        StringLiteral sla = "appended string literal";
        sint16 s16a = 12413;
        ...
        experimental :: String operated = original;
        this->log("Operator + :");
    ...
    this->executeSubtest("Find Tests", [this, &allOk] {
        experimental :: String str = "This is a sample string";
        this->log("Original String : '%s'", str.cStr());
        ...
        this->log("First 'x' index : %d", str.findFirst('x'));
        if ( str.findFirst('x') != experimental :: String::invalidIndex ) {
            this->logWarning("Find First non-existent error");
            allOk = false;
        }
        ...
        this->log("First 'isn't' index : %d", str.findFirst("isn't"));
        if ( str.findFirst("isn't") != experimental :: String::invalidIndex ) {
            this->logWarning("Find First Error");
            allOk = false;
        }
        ...
        this->log("Last 'x' index : %d", str.findLast('x'));
        if ( str.findLast('x') != experimental :: String::invalidIndex ) {
            this->logWarning("Find Last Char error");
            allOk = false;
        }
        ...
        this->log("Last 'isn't' index : %d", str.findLast("isn't"));
        if ( str.findLast("isn't") != experimental :: String::invalidIndex ) {
            this->logWarning("Find Last String error");
            allOk = false;
        }
        ...
        this->log("First of 'x', 'y' : %d", str.findFirstOf("xy"));
        if ( str.findFirstOf("xy") != experimental :: String::invalidIndex ) {
            this->logWarning("Find First Of error");
            allOk = false;
        }
        ...
        this->log("First of 'x', 'y' : %d", str.findLastOf("xy"));
        if ( str.findLastOf("xy") != experimental :: String::invalidIndex ) {
            this->logWarning("Find Last Of error");
            allOk = false;
        }
    ...
    this->executeSubtest("Substring Tests", [this, & allOk] {
        experimental :: String original = "This is a sample string";
    ...
    this->executeSubtest("Comparison Tests", [this, &allOk]{
        this->log("String(Test String) == String(Test String) : %s", experimental :: String("Test String") == experimental :: String("Test String") ? "true" : "false");
        if ( experimental :: String("Test String") != experimental :: String("Test String") ) {
            this->logWarning("String == String error");
            allOk = false;
        }

        this->log("experimental :: String(Test String) == String(Not Test String) : %s", experimental :: String("Test String") == experimental :: String("Not Test String") ? "true" : "false");
        if ( experimental :: String("Test String") == experimental :: String("Not Test String") ) {
            this->logWarning("String == String error");
            allOk = false;
        }

        this->log("experimental :: String(a) == 'a' (char) : %s", experimental :: String("a") == 'a' ? "true" : "false");
        if ( experimental :: String("a") != 'a' ) {
            this->logWarning("String == char error");
            allOk = false;
        }

        this->log("String(a) == 'b' (char) : %s", experimental :: String("a") == 'b' ? "true" : "false");
        if ( experimental :: String("a") == 'b' ) {
            this->logWarning("String == char error");
            allOk = false;
        }
...
#define COMP_TEST(_operand, _left, _right) \
            this->log(                      \
                    "String (%s) " #_operand " String (%s) : %s", \
                    experimental :: String(_left).cStr(),   \
                    experimental :: String(_right).cStr(),  \
                    experimental :: String(_left) _operand experimental :: String(_right) ? "true" : "false"\
            );                             \
                                           \
            if ( experimental :: String(_left) _operand experimental :: String(_right) != std::strcmp(_left, _right) _operand 0 ) { \
                this->logWarning("String " #_operand " compare error");                             \
                allOk = false;\
            }                              \
                                           \
            this->log(                     \
                    ...
                    experimental :: String(_left).cStr(),  \
                    _right,                \
                    experimental :: String(_left) _operand _right ? "true" : "false"\
            );                             \
                                           \
            if ( experimental :: String(_left) _operand _right != std::strcmp(_left, _right) _operand 0) {          \
                this->logWarning("String " #_operand " StringLiteral compare error");               \
                allOk = false;\
            }                              \
                                           \
            this->log(                     \
                    ...
                    experimental :: String(_left).cStr(),  \
                    std::string(_right).c_str() ,                \
                    experimental :: String(_left) _operand std::string(_right) ? "true" : "false"\
            );                             \
                                           \
            if ( experimental :: String(_left) _operand std::string(_right) != std::strcmp(_left, _right) _operand 0) {          \
                this->logWarning("String " #_operand " std::string compare error");               \
                allOk = false;\
            }\
...
#undef COMP_TEST
    ...
    this->executeSubtest("Utility Functions, String Formatting", [this, & allOk] {
        this->log("String * int ( string multiplication ) : %s * %d = %s", experimental :: String("abc...").cStr(), 5,
                  (experimental :: String("abc...") * 5).cStr());
        if (experimental :: String("abc...") * 5 != "abc...abc...abc...abc...abc...") {
            this->logWarning("String Multiplication Warning");
            allOk = false;
        }
        ...
        this->log("'Sample String' left justified by %d : '%s'", 10, experimental :: String("Sample String").ljust(10).cStr());
        this->log("'Sample String' left justified by %d : '%s'", 15, experimental :: String("Sample String").ljust(15).cStr());
        this->log("'Sample String' left justified by %d : '%s'", 30, experimental :: String("Sample String").ljust(30).cStr());

        this->log("'Sample String' right justified by %d : '%s'", 10, experimental :: String("Sample String").rjust(10).cStr());
        this->log("'Sample String' right justified by %d : '%s'", 15, experimental :: String("Sample String").rjust(15).cStr());
        this->log("'Sample String' right justified by %d : '%s'", 30, experimental :: String("Sample String").rjust(30).cStr());

        if (experimental :: String("Sample String").ljust(10).length() != std::max(experimental :: String("Sample String").length(), 10llu)) {
            this->logWarning("Left Justify Error");
            allOk = false;
        }
        if (experimental :: String("Sample String").ljust(15).length() != std::max(experimental :: String("Sample String").length(), 15llu)) {
            this->logWarning("Left Justify Error");
            allOk = false;
        }
        if (experimental :: String("Sample String").ljust(30).length() != std::max(experimental :: String("Sample String").length(), 30llu)) {
            this->logWarning("Left Justify Error");
            allOk = false;
        }

        if (experimental :: String("Sample String").rjust(10).length() != std::max(experimental :: String("Sample String").length(), 10llu)) {
            this->logWarning("Right Justify Error");
            allOk = false;
        }
        if (experimental :: String("Sample String").rjust(15).length() != std::max(experimental :: String("Sample String").length(), 15llu)) {
            this->logWarning("Right Justify Error");
            allOk = false;
        }
        if (experimental :: String("Sample String").rjust(30).length() != std::max(experimental :: String("Sample String").length(), 30llu)) {
            this->logWarning("Right Justify Error");
            allOk = false;
        }

        this->log("'%s' lowercase : '%s'", "Sample String", experimental :: String("Sample String").toLower().cStr());
        this->log("'%s' uppercase : '%s'", "Sample String", experimental :: String("Sample String").toUpper().cStr());

        if (experimental :: String("Sample String").toLower() != "sample string") {
            this->logWarning("Lower Error");
            allOk = false;
        }
        if (experimental :: String("Sample String").toUpper() != "SAMPLE STRING") {
            this->logWarning("Upper Error");
            allOk = false;
        }

        this->log("'%c' is uppercase : '%s'", 'A', experimental :: StringUtils < char > :: isUpper('A') ? "true" : "false");
        this->log("'%c' is uppercase : '%s'", 'a', experimental :: StringUtils < char > :: isUpper('a') ? "true" : "false");
        this->log("'%c' is uppercase : '%s'", '0', experimental :: StringUtils < char > :: isUpper('0') ? "true" : "false");
    ...
    this->executeSubtest("Assignment Tests", [this, & allOk] {
        experimental :: String a = "Test String";
        std::string b = "Test std::string";
        StringLiteral c = "Test StringLiteral";
        char d = 'c';

        experimental :: String o;
    ...
    this->executeSubtest("Split Tests", [this, &allOk] {
        experimental :: String toSplit("This is a sample string, using several separators. Will it be okay? I hope so! Time to split");
    ...
    this->executeSubtest("Replace Tests", [this, &allOk] {
        ...
        experimental :: String m = original;
        this->log("Replaced in '%s' from 2 to 5 with 'inserted value' : '%s'", original.cStr(), m.replace(2, 3, "inserted value").cStr());
    ...
    this->executeSubtest("Iterable Lambda Utilities Tests", [this, &allOk] {
        ...
        {
            experimental :: String toApplyTo = "Test String";
            int sum = 0;
        ...
        {
            const experimental :: String toApplyTo = "Test String";
            int sum = 0;
