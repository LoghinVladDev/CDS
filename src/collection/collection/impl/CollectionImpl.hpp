...
/// \tparam __ElementType type of elements contained in the collection
/// \tparam __LastType type of the value to be inserted
...
template <typename __ElementType, typename __LastType> __CDS_MaybeUnused inline auto __collectionAdd (  // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
...
/// \tparam __ElementType type of elements contained in the collection
/// \tparam __FirstType type of the first value in the pack to be inserted
/// \tparam __RemainingTypes type pack of the remaining values to be inserted
/// \param [out] collection : Collection ref = Reference to the collection to insert the value in
...
/// \namespace cds::experimental::__hidden::__impl
/// \internal library-private
template <typename __ElementType, typename __FirstType, typename... __RemainingTypes> inline auto __collectionAdd ( // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
    Collection <__ElementType>& collection,
...
template <typename __ElementType> // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
constexpr Collection <__ElementType>::Collection (Collection const& collection) noexcept = default;


template <typename __ElementType> // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
constexpr Collection <__ElementType>::Collection (Collection&& collection) noexcept = default;
...
template <typename __ElementType> // NOLINT(bugprone-reserved-identifier, cert-dcl37-c, cert-dcl51-cpp)
auto Collection <__ElementType>::equals (Object const& object) const noexcept -> bool {
  ...
  if (size() != pObject->size()) {
    return false;
  }

  // If sizes are equal, compare each element
  for (
      auto leftIt = cbegin(), leftEnd = cend(), rightIt = pObject->cbegin();  // NOLINT(clion-misra-cpp2008-8-0-1)
