...
CDS_ATTR(2(nodiscard, constexpr(14))) auto operator==(L&& lhs, R&& rhs) noexcept -> bool {
  ...
  auto const b1 = LA::data(cds::forward<L>(lhs));
  auto const l1 = LA::length(cds::forward<L>(lhs));
  auto const b2 = RA::data(cds::forward<R>(rhs));
  auto const l2 = RA::length(cds::forward<R>(rhs));
  if (l1 != l2) {
    return false;
  }
...
template <typename L, typename R, EnableIf<Le<Int<1>, Count<strop::UsesStringStrop, L, R>>> = 0>
CDS_ATTR(2(nodiscard, constexpr(14))) auto operator>(L&& lhs, R&& rhs) noexcept -> bool {
  using LA = StringAbstract<L>;
  using RA = StringAbstract<R>;
  auto const b1 = LA::data(cds::forward<L>(lhs));
...
private:
  template <typename L, typename R, EnableIf<Le<Int<1>, Count<strop::UsesStringStrop, L, R>>>>
