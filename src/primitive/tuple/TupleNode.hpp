...
namespace cds { // NOLINT(modernize-concat-nested-namespaces)
    ...
    namespace __hidden { // NOLINT(modernize-concat-nested-namespaces)
        namespace __impl {
            ...
            template < uint32 __index, typename __CurrentType, typename ... __TypePack >
            struct __TypeAt {
                using Type = typename __TypeAt < __index - 1, __TypePack ... > :: Type;
            };

            template < typename __CurrentType, typename ... __TypePack >
            struct __TypeAt < 0, __CurrentType, __TypePack ... > {
                using Type = __CurrentType;
            };

            template < uint32 __index, typename ... __TypePack >
            class __TupleNode {
            public:
            ...
            template < uint32 __index, typename __CurrentType, typename ... __RemainingTypes >
            ...
            public:
                template < typename __CurrentConstructorType >
                explicit __TupleNode (
                        __CurrentConstructorType && currentArgument
                ) noexcept;

            public:
                template < typename __CurrentConstructorType, typename ... __RemainingConstructorTypes >
                explicit __TupleNode (
                        __CurrentConstructorType      &&     currentArgument,
            ...
            public:
                template < uint32 __requestedIndex, meta :: EnableIf < __requestedIndex == __index > = 0 >
            ...
            public:
                template < uint32 __size = sizeof ... ( __RemainingTypes ), meta :: EnableIf < __size == 0 > = 0 >
                auto toString (
                        std :: ostream & out
                ) const noexcept -> void;
